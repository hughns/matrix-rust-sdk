...
pub use matrix_sdk_base::crypto::types::qr_login::{
    LoginQrCodeDecodeError, QrCodeData, QrCodeIntent,
};
...
pub struct CheckCodeSender {
    ...
    expected_check_code: u8,
}

impl CheckCodeSender {
    pub(crate) fn new(tx: tokio::sync::oneshot::Sender<u8>, expected_check_code: u8) -> Self {
    ...
    // TODO: we should ideally be keeping the expected_check_code within the channel itself.
    pub fn validate(&self, check_code: u8) -> bool {
        self.expected_check_code == check_code
    }
}
