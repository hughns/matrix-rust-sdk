...
async fn finish_login_grant<Q>(
    ...
    if !matches!(client.device_exists(device_id.to_owned().into()).await, Ok(false)) {
        channel
            .send_json(QrAuthMessage::LoginFailure {
    ...
    loop {
        if matches!(client.device_exists(device_id.to_owned().into()).await, Ok(true)) {
            break;
        } else {
...
impl<'a> IntoFuture for GrantLoginWithGeneratedQrCode<'a> {
    ...
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            ...
            self.state.set(GrantLoginProgress::EstablishingSecureChannel(
                GeneratedQrProgress::QrScanned(CheckCodeSender::new(tx, channel.get_check_code())),
            ));
