...
impl<'a> IntoFuture for LoginWithGeneratedQrCode<'a> {
    ...
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            ...
            let homeserver_base_url = match message {
                QrAuthMessage::LoginProtocols { protocols, base_url } => {
            ...
            // secure channel.
            if self.client.homeserver() != homeserver_base_url {
                self.client
                    .switch_homeserver_and_re_resolve_well_known(homeserver_base_url)
                    .await
...
mod test {
    ...
    async fn test_qr_login() {
        ...
        let base_url = &alice.qr_code_data().base_url;
    ...
    async fn test_generated_qr_login() {
        ...
        let homeserver_url = rendezvous_server.base_url.clone();
        ...
        assert_eq!(QrCodeIntent::Login, secure_channel.qr_code_data().intent);
    ...
    async fn test_failure(
        ...
        assert_eq!(QrCodeIntent::Reciprocate, alice.qr_code_data().intent);

        let bob = Client::builder()
            .homeserver_url(alice.qr_code_data().base_url.as_str())
            .request_config(RequestConfig::new().disable_retry())
            .build()
            .await
            .expect("We should be able to build the Client object from the URL in the QR code");
