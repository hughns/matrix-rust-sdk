...
pub struct MemoryStore {
    ...
    inbound_group_sessions_backed_up_to: StdRwLock<HashMap<OwnedRoomId, HashMap<String, String>>>,
...
impl MemoryStore {
    ...
    async fn get_inbound_group_sessions_and_backed_up_to(
        &self,
    ) -> Result<Vec<(InboundGroupSession, String)>> {
        ...
        Ok(self
            ...
            .into_iter()
            .map(|s| {
                let b = lookup(&s).unwrap_or_else(|| "".to_owned());
                (s, b)
            })
            .collect())
    }
...
impl CryptoStore for MemoryStore {
    ...
    async fn inbound_group_sessions_for_backup(
        ...
        Ok(self
            ...
            .into_iter()
            .filter_map(
                |(session, backed_up_to)| {
                    if backed_up_to != backup_version {
                        Some(session)
                    } else {
                        None
                    }
        ...
        for (room_id, session_id) in room_and_session_ids {
            ...
            if let Some(session) = session {
                ...
                inbound_group_sessions_backed_up_to
                    ...
                    .and_modify(|s| *s = backup_version.to_owned())
                    .or_insert_with(|| backup_version.to_owned());
...
mod tests {
    ...
    /// the store.
    async fn backed_up_tos(store: &MemoryStore) -> HashMap<String, String> {
        store
            .get_inbound_group_sessions_and_backed_up_to()
            .await
            .expect("Unable to get inbound group sessions and backup order")
            .iter()
            .map(|(s, o)| (s.session_id().to_owned(), o.clone()))
            .collect()
    }
}
