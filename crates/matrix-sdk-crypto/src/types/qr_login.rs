...
impl QrCodeData {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, LoginQrCodeDecodeError> {
        ...
        // 7. Two bytes for the length of the server name, a u16 in big-endian
        //    encoding.
        ...
        if qr_type == TYPE {
            // 3. The intent is the next one to parse, we return an error immediately
            //    the intent isn't 0x00 or 0x01.
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let encoded = [
            ...
            &rendezvous_id_len,
            self.rendezvous_id.as_str().as_bytes(),
            &server_name_len,
            self.server_name.as_str().as_bytes(),
        ]
        .concat();
...
mod test {
    ...
    const QR_CODE_DATA_RECIPROCATE: &[u8] = &[
        0x49, 0x4F, 0x5F, 0x45, 0x4C, 0x45, 0x4D, 0x45, 0x4E, 0x54, 0x5F, 0x4D, 0x53, 0x43, 0x34, 0x31, 0x30, 0x38,
        0x03, 0x01,
        0xd8, 0x86, 0x68, 0x6a, 0xb2, 0x19, 0x7b, 0x78, 0x0e, 0x30, 0x0a, 0x9d, 0x4a, 0x21, 0x47, 0x48, 0x07, 0x00, 0xd7, 0x92, 0x9f, 0x39, 0xab, 0x31, 0xb9, 0xe5, 0x14, 0x37, 0x02, 0x48, 0xed, 0x6b,
        0x00, 0x24,
        0x65, 0x38, 0x64, 0x61, 0x36, 0x33, 0x35, 0x35, 0x2D, 0x35, 0x35, 0x30, 0x62, 0x2D, 0x34, 0x61, 0x33, 0x32, 0x2D, 0x61, 0x31, 0x39, 0x33, 0x2D, 0x31, 0x36, 0x31, 0x39, 0x64, 0x39, 0x38, 0x33, 0x30, 0x36, 0x36, 0x38,
        0x00, 0x0A,
        0x6d, 0x61, 0x74, 0x72, 0x69, 0x78, 0x2e, 0x6f, 0x72, 0x67,
    ];

    // Test vector for the QR code data, copied from the MSC, with the intent set to login
    ...
    fn parse_qr_data_reciprocate() {
        ...
        assert_eq!(
            QrCodeIntent::Reciprocate, data.intent,
